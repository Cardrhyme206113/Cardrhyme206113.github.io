...
function sequenceWindowSafe(variant,segments,startIndex,previous,config){
  ...
  for(const hand of ["left","right"]){
    ...
    const reset=wristResetDegrees(last.direction,first.note.direction);
    const travel=sequenceCutTravel(last,first.note);
...
(async()=>{
  ...
  const newPool=[
    "    const classicPool=FLOW_PATTERNS.filter(",
    "      pattern=>pattern.difficulty===bestTier",
    "    );",
    ...
    "    )/4294967296;",
    "    const pool=(sequencePool.length&&sequenceRoll<sequenceChance)?sequencePool:classicPool;",
    "    const chosen=pool[",
    "      hashString(",
    "        `${seed}|${config.id}|${item.signature}|${bestTier}|flow`",
    "      )%pool.length",
